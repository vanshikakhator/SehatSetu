...
export default function PaymentModal({ doctor, user, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    const fetchSlots = async () => {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || (import.meta.env.VITE_API_URL || "http://localhost:5000")}/api/appointments/doctor/${doctor._id}/slots?date=${date}`);
  ...
  const handleSimulatedPayment = async () => {
    ...
    try {
      ...
      const res = await axios.post((import.meta.env.VITE_API_URL || "http://localhost:5000") + '/api/appointments/book', bookingPayload);
