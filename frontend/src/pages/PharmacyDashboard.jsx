        ...
        <nav style={{ background: COLORS.surface, borderBottom: `1px solid ${COLORS.border}`, padding: "0 5%", display: "flex", alignItems: "center", justifyContent: "space-between", height: 80, position: "sticky", top: 0, zIndex: 100 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 15 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontWeight: 800, fontSize: 28, color: COLORS.text }}>सेहतSetu <span style={{ fontSize: 16, color: COLORS.textMuted, fontStyle: "italic", fontWeight: 600 }}>Pharmacy</span></span>
              {user?.isVerified && <Badge color="success">✓ Verified</Badge>}
            </div>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <p style={{ margin: 0, fontWeight: 600, fontSize: 16, color: COLORS.text }}>{user?.name || "Sharma Medical"}</p>
            <LanguageSwitcher />
        ...
        <div className="dashboard-layout">
          ...
          <aside className="sidebar" style={{ width: 300 }}>
            <div style={{ padding: "24px", borderBottom: `1px solid ${COLORS.border}`, marginBottom: 15 }}>
              <p style={{ margin: 0, fontWeight: 800, color: COLORS.primary, fontSize: 24 }}>SehatSetu Pharmacy</p>
            </div>
            {tabs.map(t => (
              <button
                key={t.id}
                ...
                style={{
                  ...
                  borderRight: tab === t.id ? "4px solid " + COLORS.primary : "4px solid transparent"
                }}
              >
                {t.icon} {t.label}
              </button>
          ...
          <main className="main-content" style={{ padding: 40 }}>
            {tab === "inventory" && (
              <div>
                ...
                <Card style={{ marginBottom: 24, padding: 24, border: `2px solid ${locationStr ? '#22c55e' : '#f59e0b'}`, background: locationStr ? '#75a8c8ff' : '#83cbddff' }}>
                  <p style={{ margin: '0 0 12px', fontWeight: 800, fontSize: 18, color: 'black' }}>📍 Pharmacy Location (used for "Get Directions" on patient app)</p>
                  <p style={{ margin: '0 0 16px', fontSize: 14, color: 'black' }}>
                    {locationStr ? `Currently saved: ${locationStr}` : '⚠️ No location saved — patients cannot find you on the map!'}
                  </p>
                  {locationStatus && <p style={{ margin: '0 0 14px', fontSize: 14, fontWeight: 600, color: locationStatus.startsWith('✅') ? '#192223ff' : locationStatus.startsWith('❌') ? '#dc2626' : '#b45309' }}>{locationStatus}</p>}
                ...
                {inventory.map((m, i) => (
                  <div key={i} style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 16, padding: "20px 24px", marginBottom: 15, display: "flex", alignItems: "center", gap: 24 }}>
                    <div style={{ flex: 1 }}>
                ...
                {orders.length === 0 ? (
                  ...
                  orders.map(order => (
                    <Card key={order._id} style={{ padding: 24, marginBottom: 20 }}>
                      ...
                      <div style={{ background: '#566d84ff', padding: 15, borderRadius: 12, marginBottom: 20 }}>
